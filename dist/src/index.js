...
async function getAccessToken() {
    ...
    if (!request.ok)
        throw new Error(`Failed to get access token -> ${request.statusText}`);
    const response = await request.json();
    return response.access_token;
}
async function getShipments(accessToken, limit = 24, offset = 0) {
    ...
    if (!request.ok)
        throw new Error(`Failed to get shipments -> ${request.statusText}`);
    const response = await request.json();
    return response;
}
async function getShipmentById(accessToken, shipmentId) {
    ...
    if (!request.ok)
        throw new Error(`Failed to get shipment details -> ${request.statusText}`);
    const response = await request.json();
    return response;
}
async function saveShipmentsJson(accessToken, pagesCount = 50) {
    ...
    while (true) {
        ...
        if (shipmentsResponse.details.pagination.totalRecordsInPage < 24)
            break;
...
async function saveShipmentsDetailsJson(accessToken) {
...
function parseShipments(shipmentsJson) {
    const shipments = JSON.parse(shipmentsJson);
    return shipments.data.map(shipment => ShipmentSchema.parse(shipment));
}
function parseShipmentDetails(shipmentDetailsJson) {
    const shipmentDetails = JSON.parse(shipmentDetailsJson);
